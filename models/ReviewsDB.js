...
{ 
    ...
    getReviewsByRestaurant(request, respond) { 
        var sql = 'SELECT review_id, users.user_id, user_firstname, user_lastname, user_address, user_photo, review_rating, review_title, review_writeup, review_photo_1, review_photo_2, review_visited, review_added, review_status, review_sentiment FROM eatout.reviews INNER JOIN eatout.users ON users.user_id = reviews.user_id INNER JOIN eatout.restaurant ON restaurant.restaurant_id = reviews.restaurant_id WHERE restaurant.restaurant_id = ?';
        var userId = request.params.id;
    ...
    createReview(request, respond) { 
        ...
        var sql = 'INSERT INTO eatout.reviews (restaurant_id, user_id, review_rating, review_title, review_writeup, review_photo_1, review_photo_2, review_visited, review_sentiment) VALUES (?,?,?,?,?,?,?,?,?)';
        var values = [request.body.restaurant_id, request.params.id, request.body.review_rating, request.body.review_title, request.body.review_writeup, request.body.review_photo_1, request.body.review_photo_2, request.body.review_visited, request.body.review_sentiment];
